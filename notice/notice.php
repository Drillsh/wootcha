...
?>
				<li id="member_list">
					...
					<span class="col2"><a id="noticetitle" href="notice_view.php?num=<?=$num?>&page=<?=$page?>&hit=<?=$hit+1?>"><?=$title?></a></span>
		...
		$page_scale = 5; // 페이지 쪽수 표시 량 (5 페이지씩 표기)
			   $pageGroup = ceil($page / $page_scale); // 페이지 그룹번호(페이지 5개가 1그룹)
				...
				$last_page = $total_page;
					...
					$url = "/wootcha/notice/notice.php?y=$y&m=$m";
					if ($search != '') {
					...
					if ($first_page == 1) {
						...
						echo "<li><a href='$url&page=1'><span class='page_num_direction'><i class='fas fa-angle-double-left'></i></span></a></li>";							echo "<li><a href='$url&page=$prev'><span class='page_num_direction'><i class='fas fa-angle-left'></i></span></a></li>";
						}
						//페이지 번호 매기기
					...
					echo "<li><a><span class='page_num_direction'><i class='fas fa-angle-right'></i></span></a></li>";
						...
						?>
							...
							<div id="listwrite">
								...
								?>
								<input class="listwrite" type="image" src="./img/write.png" onclick="location.href='notice_form.php'">
							<?php
							}
							?>
							</div>
