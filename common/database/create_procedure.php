          ...
          case 'get_join_g_members' :
            ...
            BEGIN 
              ...
              WHERE
                ...
                LEFT JOIN
              (SELECT 
                `user_signup_day` AS `date`, COUNT(*) AS `count`
              FROM
                user
              WHERE
                DATE(`user_signup_day`) >= STR_TO_DATE(start_date, '%Y-%m-%d') AND DATE(`user_signup_day`) <= STR_TO_DATE(last_date, '%Y-%m-%d')
        ...
        case "get_day_reivew_for_one_month":
          ...
          BEGIN 
            ...
            WHERE
              ...
              LEFT JOIN
            (SELECT 
              `review_regtime` AS `date`, COUNT(*) AS `count`
            FROM
              review
            WHERE
              DATE(`review_regtime`) >= STR_TO_DATE(start_date, '%Y-%m-%d') AND DATE(`review_regtime`) <= STR_TO_DATE(last_date, '%Y-%m-%d')
